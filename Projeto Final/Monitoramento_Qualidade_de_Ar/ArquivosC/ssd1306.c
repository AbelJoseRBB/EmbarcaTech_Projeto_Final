...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    ...
    for (uint8_t i = 0; i < 8; i++) { // Itera sobre as 8 linhas do caractere
        uint8_t line = char_data[i]; // Obtém a linha atual do caractere
        for (uint8_t j = 0; j < 8; j++) { // Itera sobre os 8 bits da linha
            if (line & 0x01) { // Verifica o bit menos significativo
